...
public class Elevador : MonoBehaviour
{
    ...
    {
        //PosicionBaja
        Pos2 = WP2.transform.localPosition;
        Pos1 = WP1.transform.localPosition;

        Vector3 direction = Pos1 - Pos2;
        float distancia = Vector3.Distance(Pos1, transform.localPosition);
        if (distancia > 0.5f)
        {
            Posicion2?.Invoke();
    ...
    void move2()
    {
        ...
        float distancia = Vector3.Distance(Pos2, transform.localPosition);
        if (distancia > 0.5f)
        {
            Posicion2?.Invoke();
            transform.Translate(direction * speed * Time.deltaTime);
